...
| `_id`                    | ObjectId   | Generated by MongoDB                      |
| `userId`                 | ObjectId   | Required, unique reference to `users`     |
| `productIds`             | ObjectId[] | Up to 100 unique references to `products` |
| `createdAt`, `updatedAt` | Date       | Managed by Mongoose                       |

Wishlist reads return only active current product documents. Unlike order items, wishlist entries are references rather than snapshots because current price, stock, and availability must be shown.
...
| wishlists  | `userId` unique                       | One wishlist per user             |
